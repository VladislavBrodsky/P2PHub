...
export const UpgradeButton = ({ onClick, className = '', showPrice = true }: UpgradeButtonProps) => {
    ...
    return (
        <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            onClick={handleClick}
            className={`relative w-full group overflow-hidden rounded-full p-px ${className}`}
        >
            ...
            {/* Inner Content */}
            <div className="relative h-full bg-(--card-bg) rounded-full flex items-center justify-between px-4 py-3 group-hover:bg-slate-900/5 dark:group-hover:bg-white/5 transition-colors">
