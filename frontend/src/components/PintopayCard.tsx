...
export const PintopayCard = ({
    ...
    const cardColors = {
        ...
        physical: 'bg-gradient-to-br from-slate-800 to-black',
        platinum: 'bg-gradient-to-br from-slate-300 to-slate-500',
    };
    ...
    return (
        ...
        >
            <motion.div
                ...
                className={`group relative h-full w-full overflow-hidden rounded-xl shadow-2xl transition-all duration-300 ${cardColors[variant]}`}
            >
                {/* Card Background Generic Pattern */}
                ...
                <div className="absolute top-6 left-6 z-20 opacity-80">
                    <Wifi className="h-8 w-8 text-white rotate-90" strokeWidth={2} />
                </div>
                ...
                <div className="absolute bottom-6 left-6 z-20 transition-transform duration-300 group-hover:scale-110 drop-shadow-xl">
                    <span className="text-xl font-black text-white tracking-tighter">Pintopay</span>
                </div>
