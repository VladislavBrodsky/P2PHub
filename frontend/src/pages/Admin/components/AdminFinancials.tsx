    ...
    return (
        ...
        >
            <div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 flex items-start gap-3">
                ...
                <div>
                    <h3 className="text-xs font-bold text-emerald-500 uppercase tracking-widest">{t('admin:financials.title')}</h3>
                    <p className="text-label text-slate-500 font-medium mt-1">{t('admin:financials.desc')}</p>
                </div>
            </div>
            ...
            <div className="p-6 rounded-[2.5rem] bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 space-y-6 relative overflow-hidden">
                ...
                <div className="space-y-1 relative z-10">
                    <span className="text-blue-600 dark:text-blue-400 text-label font-bold uppercase tracking-widest">{t('admin:financials.profit_net')}</span>
                    <div className="flex items-end justify-between">
                        ...
                        <div className="text-right">
                            <div className="text-label font-bold text-slate-500 uppercase">{t('admin:financials.gross_margin')}</div>
                            <div className="text-xl font-bold text-emerald-500">{stats?.financials.gross_margin}%</div>
                        </div>
                    </div>
                    <p className="text-slate-500 dark:text-slate-500 text-label font-bold">{t('admin:financials.revenue_retained')}</p>
                </div>

                <div className="grid grid-cols-2 gap-4 relative z-10 pt-4 border-t border-slate-200 dark:border-white/5">
                    <div>
                        <div className="text-slate-500 text-label font-bold uppercase">{t('admin:financials.paid_referral')}</div>
                        <div className="text-lg font-bold text-slate-900 dark:text-white">${stats?.financials.total_commissions}</div>
                        <div className="text-label text-blue-600 dark:text-blue-400 font-bold">{stats?.financials.actual_payout_ratio}{t('admin:financials.revenue_of')}</div>
                    </div>
                    <div>
                        <div className="text-slate-500 text-label font-bold uppercase">{t('admin:financials.target_split')}</div>
                        <div className="text-lg font-bold text-slate-400 dark:text-slate-400">{stats?.financials.theoretical_payout_ratio}%</div>
                        <div className="text-label text-slate-400 dark:text-slate-600 font-bold italic">{t('admin:financials.max_distribution')}</div>
                    </div>
                </div>

                <div className="grid grid-cols-2 gap-4 relative z-10 pt-4 border-t border-slate-200 dark:border-white/5">
                    <div>
                        <div className="text-slate-500 text-label font-bold uppercase">{t('admin:financials.gross_revenue')}</div>
            ...
            <div className="p-5 rounded-3xl glass-panel-premium border border-black/5 dark:border-white/5 space-y-4">
                <div className="flex items-center justify-between">
                    <h2 className="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">{t('admin:financials.performance')}</h2>
            ...
            <div className="space-y-3">
                <h2 className="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 px-1">{t('admin:financials.recent_sales')}</h2>
            ...
            <div className="space-y-3">
                <div className="flex items-center justify-between px-1">
                    <h2 className="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">{t('admin:financials.commission_split')}</h2>
                    <PieChart size={14} className="text-slate-500" />
                </div>
                <div className="space-y-2">
