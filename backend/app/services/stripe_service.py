...
class StripeService:
    async def create_checkout_session(
        ...
        if plan == "PRO":
            ...
            price_id = settings.STRIPE_UPGRADE_PRICE_ID if is_upgrade else settings.STRIPE_PRO_PLUS_PRICE_ID
        else:
        ...
        try:
            # 2026 Best Practice: Using automatic payment methods and allowing promo codes
            session = stripe.checkout.Session.create(
                payment_method_types=['card'], # Can be expanded to ['card', 'link', 'revolut_pay'] if needed
                line_items=[{
                    'price': price_id,
                    'quantity': 1,
                }],
                ...
                allow_promotion_codes=True, # Added for marketing flexibility
                metadata={
